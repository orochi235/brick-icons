...
def brush():
    ...
    ferrule: 1.5 units tall with its base where the old sweep put it."""
    svg = (pathlib.Path(__file__).resolve().parent / "bristles.svg").read_text()
    d = re.search(r'class="st1"[^>]*\sd="([^"]+)"', svg).group(1)
    pts = sample(d)
    xs = [p[0] for p in pts]
    ys = [p[1] for p in pts]
...
if __name__ == "__main__":
    ...
    cuts = contract(brush_pts, 0.0, Point(tip_xy).buffer(260.0))
