...
def brush():
    ...
    for i in range(n):
        ...
        widths.append(max(200.0 * math.sin(math.pi * u ** 0.6) ** 0.7, 3.0))
    ...
    ys = [p[1] for p in pts]
    cx = (min(xs) + max(xs)) / 2
    return [(px - cx, py) for px, py in pts]
...
if __name__ == "__main__":
    ...
    tip = -470.0
    cuts = contract(magnet_pts, 34.0, sbox(-GRID, tip, GRID, tip + 210.0))
