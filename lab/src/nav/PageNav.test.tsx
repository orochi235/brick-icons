...
it('links to the other page by href, not a handler', () => {
  render(<PageNav path="/corpus" />);
  expect(screen.getByRole('link', { name: 'Dashboard' })
    .getAttribute('href')).toBe('/stats');
});

it('still highlights through a query string or a trailing slash', () => {
  expect(currentIndex('/corpus?source=silhouette-occt')).toBe(0);
  expect(currentIndex('/stats#tiles')).toBe(1);
});

it('still highlights an old .html bookmark', () => {
  expect(currentIndex('/corpus.html')).toBe(0);
  expect(currentIndex('/stats.html?source=naive')).toBe(1);
});

it('highlights nothing on a page that is not in the list', () => {
  expect(currentIndex('/badges')).toBe(-1);
  render(<PageNav path="/badges" />);
