...
**Unbuilt as of 2026-09-09.** Designed and not implemented, deliberately: two
fleet jobs were feeding the ingest watcher every five minutes, and changing
what a run row means while one is being written is the wrong hour for it. Do
it when `slot-occt-r1` and `slot-occt-r2` have landed and the watcher is
stopped.

For whoever implements it. You know the repo; this says what to change and
what not to.
