...
Expected: FAIL — TypeScript rejects the unknown `loupe` and `onHover` props, and `.pane-loupe` is never found.
...
Expected: PASS — the 12 original tests plus the 5 new ones.
...
Expected: FAIL — `onFactor` is not a prop, and the Alt wheel still calls `onCamera`.
